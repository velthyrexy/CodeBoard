...
class SuggestionEngine {
    ...
    private val keywordsByLanguage = mapOf(
        ...
        "C#" to listOf(
            ...
            "enum", "void", "string", "bool", "double", "float", "var", "object", 
    ...
    fun getSuggestions(language: String, prefix: String, limit: Int = 15): List<Suggestion> {
        ...
        if (prefix.isBlank()) {
            return keywords.take(limit).map { Suggestion(text = it, type = "keyword", popularity = 1) }
        }
        return keywords.filter { it.startsWith(prefix, ignoreCase = true) }
            .take(limit)
            .map { Suggestion(text = it, type = "keyword", popularity = 1) }
    }
}
